...
with tf.Session() as sess:
    for trainstep, learningRate in trainsteps:
        ...
        #
        import numpy as np
        example_train_immages = np.array(example_train_immages)
        example_train_processed = np.array(example_train_processed)
        example_test_immages = np.array(example_test_immages)
        example_test_processed = np.array(example_test_processed)
        example_train_immages = example_train_immages + np.random.uniform(-0.1, 0.1, example_train_immages.shape)
        example_train_processed = example_train_processed + np.random.uniform(-0.1, 0.1, example_train_processed.shape)
        example_test_immages = example_test_immages + np.random.uniform(-0.1, 0.1, example_test_immages.shape)
        example_test_processed = example_test_processed + np.random.uniform(-0.1, 0.1, example_test_processed.shape)
        example_train_immages = np.minimum(np.maximum(example_train_immages, 0.0), 1.0)
        example_train_processed = np.minimum(np.maximum(example_train_processed, 0.0), 1.0)
        example_test_immages = np.minimum(np.maximum(example_test_immages, 0.0), 1.0)
        example_test_processed = np.minimum(np.maximum(example_test_processed, 0.0), 1.0)
